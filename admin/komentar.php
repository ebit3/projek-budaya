    ...
    <div class="row mb-5">
      ...
      <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <h2 class="h2"><i class="fas fa-comments me-1"></i> Komentar</h2>
        </div>
        <div class="table-responsive mt-4">
          <table class="table table-striped table-bordered text-center">
            ...
            $previous = $halaman - 1;
            $next = $halaman + 1;
            $data = mysqli_query($koneksi, "SELECT * FROM komentar, artikel WHERE komentar.id_artikel = artikel.id_artikel ORDER BY komentar.id_komentar DESC") or die(mysqli_error($koneksi));
            $jumlah_data = mysqli_num_rows($data);
            $total_halaman = ceil($jumlah_data / $batas);
            $syarat = mysqli_query($koneksi, "SELECT * FROM komentar, artikel WHERE komentar.id_artikel = artikel.id_artikel ORDER BY komentar.id_komentar DESC LIMIT $halaman_awal, $batas");
            $nomor = $halaman_awal + 1;
            while ($data = mysqli_fetch_assoc($syarat)) :
              $komentar_sebagian = substr($data['komentar'], 0, 25);
            ...
            ?>
              <tr>
                <td><?= $no++ ?></td>
                <td><?= $data['kategori'] . " - " . $data['judul'] ?></td>
                <td><?= $data['nama'] ?></td>
                <td><?= $format_tgl . " " . $format_bln . " " . $format_thn ?></td>
                <td><?= $komentar_sebagian . "..." ?></td>
                <td>
                  <a href="detail_komentar.php?id=<?= $data['id_komentar'] ?>" class="btn btn-sm btn-info"><i class="fas fa-eye"></i> Detail</a>
                  <a href="hapus_komentar.php?id=<?= $data['id_komentar'] ?>" class="btn btn-sm btn-danger"><i class="fas fa-trash-alt"></i> Hapus</a>
                </td>
              </tr>
            <?php
            endwhile;
      ...
      <?php if (isset($_GET['hapus'])) { ?>
        <div class="flash-data fd-1" data-flashdata="<?= $_GET['hapus']; ?>"></div>
      <?php } ?>
    </div>
  </div>
