...
if (isset($_POST['simpan'])) {
  ...
  if (empty($isi)) {
    header("Location: edit_budaya.php?id=" . $id . "&&gagal=1");
  } else {
    if ($gambar != "") {
      $x = explode('.', $gambar);
      ...
      if (in_array($ekstensi, $ekstensi_diperbolehkan) === true  && move_uploaded_file($file_tmp, '../assets/images/artikel/' . $gambarbaru)) {
        ...
        if ($ubah) {
          header('location: budaya.php?edit=1');
        }
      }
    } else if ($gambar = "") {
      mysqli_query($koneksi, "UPDATE artikel SET judul='$judul', asal='$asal', kategori='$kategori', penulis='$penulis', jadwal='$jadwal', tanggal='$tanggal', preview='$preview', isi='$isi', link='$link' WHERE id_artikel='$id'");
      header('location: budaya.php?edit=1');
    } else {
      mysqli_query($koneksi, "UPDATE artikel SET judul='$judul', asal='$asal', kategori='$kategori', penulis='$penulis', jadwal='$jadwal', tanggal='$tanggal', preview='$preview', isi='$isi', link='$link' WHERE id_artikel='$id'");
      header('location: budaya.php?edit=1');
