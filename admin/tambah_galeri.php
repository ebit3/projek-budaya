...
if (isset($_POST['simpan'])) {
  ...
  for ($x = 0; $x < $jumlahFile; $x++) {
    $foto = $_FILES['foto']['name'][$x];
    $f = explode('.', $foto);
    $ekstensi = strtolower(end($f));
    $ukuran = $_FILES['foto']['size'][$x];
    $tmp = $_FILES['foto']['tmp_name'][$x];
    $fotobaru = date('H-i-s-d-m-Y') . " - " . $foto;
    if (in_array($ekstensi, $ekstensi_diperbolehkan)) {
      move_uploaded_file($tmp, '../assets/images/artikel/gambarlain/' . $fotobaru);
      $simpan = mysqli_query($koneksi, "INSERT INTO gambar VALUES ('','$id_artikel','$fotobaru')");
      if ($simpan) {
        header("Location: galeri.php?tambah=1");
      }
    }
  }
}
    ...
    <div class="row">
      ...
      <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <h2 class="h2"><i class="fas fa-image me-1"></i> Tambah Galeri</h2>
        </div>
        <div class="container mb-5">
          <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group mt-2">
              ...
              <input required type="file" accept="image/*" name="foto[]" id="foto" class="form-control mt-1" multiple>
              <i class="info">*Gambar bisa lebih dari satu.</i>
            </div>
