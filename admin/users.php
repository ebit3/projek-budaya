    ...
    <div class="row mb-5">
      ...
      <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <h2 class="h2"><i class="fas fa-users me-1"></i> Users</h2>
        </div>
        <a href="tambah_user.php" class="btn btn-primary mt-2"><i class="fas fa-plus me-1"></i> Tambah</a>
        <div class="table-responsive mt-4">
          <table class="table table-striped table-bordered text-center">
            ...
            $next = $halaman + 1;
            if (isset($_POST['search'])) {
              $data = mysqli_query($koneksi, "SELECT * FROM users WHERE nama LIKE '%" . $_POST['search'] . "%' OR username LIKE '%" . $_POST['search'] . "%' ORDER BY id_user DESC") or die(mysqli_error($koneksi));
              $jumlah_data = mysqli_num_rows($data);
              $total_halaman = ceil($jumlah_data / $batas);
              $syarat = mysqli_query($koneksi, "SELECT * FROM users WHERE nama LIKE '%" . $_POST['search'] . "%' OR username LIKE '%" . $_POST['search'] . "%' ORDER BY id_user DESC LIMIT $halaman_awal, $batas");
            } else {
              $data = mysqli_query($koneksi, "SELECT * FROM users ORDER BY id_user DESC") or die(mysqli_error($koneksi));
              $jumlah_data = mysqli_num_rows($data);
              $total_halaman = ceil($jumlah_data / $batas);
              $syarat = mysqli_query($koneksi, "SELECT * FROM users ORDER BY id_user DESC LIMIT $halaman_awal, $batas");
            }
            ...
            ?>
              <tr>
                ...
                <td><?= $data['username'] ?></td>
                <td><?= $data['nama'] ?></td>
                <td><a href="detail_user.php?id=<?= $data['id_user'] ?>" class="btn btn-sm btn-info"><i class="fas fa-eye"></i> Detail</a></td>
              </tr>
            <?php
            endwhile;
            ?>
  ...
  <script type="text/javascript">
    ...
    if (flashdata) {
      Swal.fire({
        icon: 'success',
        title: 'Berhasil',
        text: 'Data User sudah ditambahkan!'
      })
    }
  </script>
</body>
