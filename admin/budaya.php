  ...
  <div class="container-fluid">
    <div class="row mb-5">
      ...
      <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        ...
        <a href="tambah_budaya.php" class="btn btn-primary mt-2"><i class="fas fa-plus me-1"></i> Tambah</a>
        <div class="table-responsive mt-4">
          <table class="table table-striped table-bordered text-center">
            ...
            $next = $halaman + 1;
            if (isset($_POST['search'])) {
              $data = mysqli_query($koneksi, "SELECT * FROM artikel WHERE kategori = 'Budaya' AND judul LIKE '%" . $_POST['search'] . "%' ORDER BY id_artikel DESC") or die(mysqli_error($koneksi));
              $jumlah_data = mysqli_num_rows($data);
              $total_halaman = ceil($jumlah_data / $batas);
              $syarat = mysqli_query($koneksi, "SELECT * FROM artikel WHERE kategori = 'Budaya' AND judul LIKE '%" . $_POST['search'] . "%' ORDER BY id_artikel DESC LIMIT $halaman_awal, $batas");
            } else {
              $data = mysqli_query($koneksi, "SELECT * FROM artikel WHERE kategori = 'Budaya' ORDER BY id_artikel DESC") or die(mysqli_error($koneksi));
              $jumlah_data = mysqli_num_rows($data);
              $total_halaman = ceil($jumlah_data / $batas);
              $syarat = mysqli_query($koneksi, "SELECT * FROM artikel WHERE kategori = 'Budaya' ORDER BY id_artikel DESC LIMIT $halaman_awal, $batas");
            }
            ...
            ?>
              <tr>
                ...
                <td>
                  <a href="detail_budaya.php?id=<?= $data['id_artikel'] ?>" class="btn btn-sm btn-info"><i class="fas fa-eye"></i> Detail</a>
                  <a href="edit_budaya.php?id=<?= $data['id_artikel'] ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i> Edit</a>
                  <a href="hapus_budaya.php?id=<?= $data['id_artikel'] ?>" class="btn btn-sm btn-danger"><i class="fas fa-trash-alt"></i> Hapus</a>
                </td>
              </tr>
            <?php
            endwhile;
