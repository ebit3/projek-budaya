	...
	class ScrollBarHelper {
		...
		_applyManipulationCallback(selector, callBack) {
			if (isElement(selector)) {
				...
				SelectorEngine.find(selector, this._element).forEach(
					callBack
				);
			}
		}
	...
	class Tooltip extends BaseComponent {
		...
		show() {
			...
			if (
				...
				this.tip &&
				this.getTitle() !==
					this.tip.querySelector(SELECTOR_TOOLTIP_INNER)
						.innerHTML
			) {
				this._disposePopper();
		...
		_fixTitle() {
			...
			const originalTitleType =
				typeof this._element.getAttribute('data-bs-original-title');
		...
		_enter(event, context) {
			...
			if (
				context
					.getTipElement()
					.classList.contains(CLASS_NAME_SHOW$2) ||
	...
	class ScrollSpy extends BaseComponent {
		...
		_clear() {
			SelectorEngine.find(SELECTOR_LINK_ITEMS, this._config.target)
				.filter((node) =>
					node.classList.contains(CLASS_NAME_ACTIVE$1)
				)
				.forEach((node) =>
